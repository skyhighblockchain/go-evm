...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"skh": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	message := "Welcome to the Skyhigh Blockchain JavaScript console!\n\n"

	// Print some generic Geth metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + skh.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + skh.blockNumber + " (" + new Date(1000 * skh.getBlock(skh.blockNumber).timestamp) + ")\n";
		try {
